...
<script>
  // 插件配置由 buildFlyoutPage 在服务端内联（宿主侧热重载后重新内联）。
  // 顶层 var 落到全局，供后续 sharedScript 中的 renderActive 等读取。
  var FLYOUT_CONFIG = ${JSON.stringify(config)};
  (function () {
...
${sharedScript}
    ...
    function renderActive() {
      ...
      // 内容区字号：取宿主内联的插件配置（FLYOUT_CONFIG，服务端热重载后
      // 重新内联），容器设基准 px，区内代码/diff/markdown 用 em 相对字号联动缩放。
      var _f = FLYOUT_CONFIG.contentFontSize;
      if (typeof _f === 'number' && _f >= 11 && _f <= 20) area.style.fontSize = _f + 'px';
      area.textContent = '';
      var t = activeTab();
      if (!t) {
